...
var current_score = [];
var current_tree = "Deployer-v2.0.0.json"; //mf Default-Input geändert
...
var change_cve_id = 0; //mf Variable für CVE-ID aus JSON-File
var cve_id = ""; //mf Variable für CVE-ID aus JSON-File
...
function dynamic_mwb(w) {
    ...
    $(pmwbid+' .wscore').html(result.replace(/_/g, ' '));
    $(pmwbid+' .wsdiv').show();
    $('circle[nameid="'+result.replace(/\s/g, '_').toLowerCase()+'"]').parent().simClick();
...
function export_show(novector) {
	...
	$('.exportId').val($('#cve_samples').val())}
		if(change_cve_id==1 && cve_id){	//mf Variable für CVE-ID aus JSON-File
			$('.exportId').val(cve_id); //setzt CVE-ID ein
			if (export_role != "Unknown") {
			$('.exportRole').val(export_role); //setzt "Set Role", wenn angegeben
			}
		}
		change_cve_id = 0;
    if(novector == true)
...
function export_json() {
    ...
    var download_filename = oexport.id+"_"+oexport.role+"_json.json"
    if (includetree) {
	...
	try {
		...
		select_add_option($('#tree_samples'),file.name);
			if(file.name.match(/\.json$/i)){
				...
				if ($('#tree_samples').val() != "import") {
					current_tree = $('#tree_samples').val() //mf Tree beim Einlesen aktualisieren
				}
			}
			else
				parse_file(reader.result)
		}
		else if(input.id == "cvejsonload") {
			if(file.name.match(/\.json$/i)){
				$('#cve_samples2 option:nth-child(n+3)').remove().length //mf CVE-Liste vor jedem Import leeren
				dt_clear(); //mf Grafik zuruecksetzen
				if (JSON.parse(reader.result).hasOwnProperty("options")){
					change_tree(JSON.parse(reader.result));
					setTimeout(function() {
						dt_start()
					}, 250); //mf Grafik starten, wenn nur eine Decision in JSON-File, Verzögerung für JSON-Verarbeitung
				}
				setTimeout(function() {
					import_json(reader.result)
				}, 250);
				}
			else
				topalert("Please select a JSON-File.","danger")
		} 
	    else
...
function change_tree(jp) { 
/* Wechselt den Entscheidungsbaum und aktiviert die onchange-Aktion */
	if(document.getElementById('keep_tree').checked) { return; } //mf Checkbox abfragen
	var dec_tree = jp.decision_tree_url.substring(jp.decision_tree_url.lastIndexOf('/')+1); //mf Tree aus Import-Datei auslesen
	var ts_length = $('#tree_samples')[0].length
	var tsamples = $('#tree_samples').get() //mf hole alle Einträge des Entscheidungsbaums
	var found = 0;

	for(var i=0; i< ts_length; i++) {
		if (tsamples[0][i].value == dec_tree) {
			found = 1
			break;
		}
	}

	if (found == 1) {	//mf Tree ist vorhanden
		$('#tree_samples').val(dec_tree)
		$('#tree_samples').trigger("change")
	} else {
		topalert("Please import the previously used Tree: "+dec_tree, "danger");
	}
}
...
function tree_process(w) {
	$('#cve_samples2').val("Choose JSON Decision") //mf setzt JSON-Auswahl bei Tree-Wechsel zurück
	$('#cve_samples').val("Choose Sample CVE") //mf setzt TSV-Auswahl zurück
	if ($('#tree_samples').val() != "import") {
		current_tree = $('#tree_samples').val() //mf Tree beim Wechsel aktualisieren
	}
	
	var ptree = $(w).val()

	if(ptree == "select"){ //mf bei Auswahl "Select" nichts unternehmen
		return;
	}

    if(ptree == "import") {
	if((navigator.userAgent.indexOf("Chrome") && navigator.userAgent.indexOf("Firefox")) < 0) {
	    $('#dtreecsvload').show()
	    $('#dtreecsvload').click()
	...
	var ptree_name = ptree.replace(/\.[^\.]+$/,'')
		// console.log(ptree) //mf Dateiname der enthaltenen JSONs
		// console.log(ptree_name) //mf Dateiname ohne Endung ".json"
...
function disable_buttons(stimes) {
	/* Disable buttons temporarily aus v5.1.5 */ //mf Disable für max. Dauer von clickprocess()
	$('.complex button').attr('disabled',1); //mf ersetzt #biscuit.fadein
	$('.complex select').attr('disabled',1); //mf 
	setTimeout(function() {$('.complex button').removeAttr('disabled');},stimes[stimes.length-1]) //mf Buttons nach Klickdauer nicht mehr ausgrauen
	setTimeout(function() {$('.complex select').removeAttr('disabled');},stimes[stimes.length-1])
}

function process(w) {
	/* CVE- und JSON-Import-Funktion */
	var cve = $(w).val()
    if(cve == "import") {
	if((navigator.userAgent.indexOf("Chrome") < 0) && (navigator.userAgent.indexOf("Firefox")) < 0) { //mf Wenn Browser-Agent-String nicht "Chrome" oder Firefox enthält (Opera, Edge, Chrome enthalten den String)
	    $('#cvetsvload').show() //mf blendet "Durchsuchen..." ein, wenn "Import" angeklickt wird
	    topalert("Choose the file to upload below")
	} else 
	    $('#cvetsvload').click()
	return
	...
	if(cve == "importjson") {
		if((navigator.userAgent.indexOf("Chrome") < 0) && (navigator.userAgent.indexOf("Firefox")) < 0) { 
			$('#cvejsonload').show() //mf blendet "Durchsuchen..." ein
			topalert("Choose the file to upload below")
		} else 
			$('#cvejsonload').click()
		return
    ...
    if(!cve_data) {
		if($(w).val() == "Choose Sample CVE"){
			return;
		} else if($(w).val() == "Choose JSON Decision"){
			return;
		} else {
	...
	if (cve_data.hasOwnProperty("options")){
		change_tree(cve_data);
		setTimeout(function() {
			dt_start()
		}, 250); //mf Grafik starten, wenn nur eine Decision in JSON-File
		setTimeout(function() {
			import_json(JSON.stringify(cve_data));
		}, 250);
		return;
	}
	...
	//mf Anpassung der veralteten und unvollständigen Parameter
    var steps = ['Exploitation','Exposure','Automatable','Value Density','Utility','Situated Safety Impact','Mission Impact','Human Impact']
    var stimes = [1500,3000,4500,5000,6000,7500,8000,9000]
	//console.log(new Date().getTime())    
	disable_buttons(stimes);
	 for(var i=0; i< steps.length; i++) {
	clickprocess(steps[i],cve_data,stimes[i])
     }
	// $('#biscuit').fadeOut(130) //mf grauer Layer verzögert die Bearbeitung bei unvollständigem Input
	/* cve_table aufbauen */
	for(var k in Object.keys(cve_data)) //mf ausgelagert aus tsv_load, damit Tabelle immer neu geladen wird und aktuell ist
		$('#cve_table thead tr').append("<th>"+Object.keys(cve_data)[k]
		.replace(/(\([^)]+\))/,
...
function clickprocess(tstep,cve_data,stime) {
    setTimeout(function() {
	if(typeof(cve_data) != "object"){	//mf Fehler bei unvollständigem/fehlerhaftem JSON-Import
		console.log("Check decision trees in imported file.");
		return;
	}
	if(tstep in cve_data) {
		var selector = '[parentname="'+tstep+'"].prechk-'+cve_data[tstep].toLowerCase().replace(/\s/g, '_').replace(/\//g, '_or_').replace(/n\'t/g, '_not')
		if($(selector).length == 1) {
			$(selector).simClick(); 
			$('.bcomplex').modal('hide');  //mf Auswahlfenster ausblenden 
		} else {
				console.log("Couldn't find the decision point: "+tstep)
		}
	} else {
...
function tsv_load(data) {
    ...
    // for(var i=0; i<heads.length;i++) //mf ausgelagert in process, damit Tabelle immer neu geladen wird und aktuell ist
	// $('#cve_table thead tr').append("<th>"+heads[i]
	// 				.replace(/(\([^)]+\))/,
	// 					 '<br><span class="text-muted">$1</span>')+
	// 				"</th>")
    if(rmv) 
	topalert("Loaded TSV CVE samples count of "+scores.length+". Please select a sample below.","success")
	$('#cvetsvload').hide()
}
...
function load_json_decision() {
    $.get("./ssvc-decisions/Advisory-ssa-306654-SSVC-tree-4.json",import_json);
}

function import_json(jraw){
	var steps = [];
	var stimes = [];
	...
	$('#cve_table thead tr th').remove() //mf leert Tabelle
	$('#cve_table tbody tr td').remove()

	if (jp.hasOwnProperty("ssvc_decision")){
		if(isNaN(jp.ssvc_decision.length)){ 
				topalert("Please check the format of the imported file.","danger");
				return; 
		}
		$('#cve_samples2 option:nth-child(n+3)').remove().length  //mf setzt Import-Listen zurück
		/* Erstellt für die SSVC-Einträge Dropdown-Menüpunkte */
		for(var i=0; i< jp.ssvc_decision.length; i++) { 
			$('#cve_samples2').append($("<option></option>")
			.attr("id",jp.ssvc_decision[i].id)
			.text(jp.ssvc_decision[i].id)
			.data(jp.ssvc_decision[i]))
		}
		topalert("Loaded JSON CVE samples count of "+jp.ssvc_decision.length+". Please select a sample below.","success") 
	} else if (jp.hasOwnProperty("options")){
		/* Füllt die Variable für die CVE-ID aus der JSON-Datei */
		cve_id = jp.id 
		export_role = jp.role
		/* Baut die Spalten 1-3 der table_toggle-Tabelle auf */
		if (jp.id){
			$('#cve_table thead tr').append("<th>"+"ID")
			$('#cve_table tbody tr').append("<td class='d-temp'>"+cve_id+"</td>")
		}
		if (jp.role){
			$('#cve_table thead tr').append("<th>"+"Role")
			$('#cve_table tbody tr').append("<td class='d-temp'>"+jp.role+"</td>")
		}
		if (jp.decision_tree_url){
			var dec_tree = jp.decision_tree_url.substring(jp.decision_tree_url.lastIndexOf('/')+1);
			var dec_tree_short = dec_tree.replace(".json","")
			$('#cve_table thead tr').append("<th>"+"Decision Tree");
			$('#cve_table tbody tr').append("<td class='d-temp'>"+dec_tree_short+"</td>")
		}
		/* Baut die restlichen Kopfzeilen der Tabelle auf */
		for(var i=0; i<jp.options.length;i++)
			$('#cve_table thead tr').append("<th>"+String(Object.keys(jp.options[i]))
							.replace(/(\([^)]+\))/,
								 '<br><span class="text-muted">$1</span>')+
							"</th>")
		/* Hängt die cve_id an JSON-Tree-Liste an */
		if (($('#cve_samples2 option:nth-child(n+3)').length) == 0) {
			$('#cve_samples2').append($("<option></option>")
			.attr("id",jp.id)
			.text(jp.id)
			.data(jp))
		}
		/* Füllt die Klickschritte und Klickzeiten */
		for(var i=0; i< jp.options.length; i++) { 
			steps[i] = String(Object.keys(jp.options[i]));
			stimes[i]=(i+1)*1600;
		}
		...
		for(var i=0; i< steps.length; i++) {
			clickprocess(steps[i],jp.options[i],stimes[i]);
			/* Baut die table_toggle-Tabelle auf */
			for(var k in jp.options[i])
				$('#cve_table tbody tr').append("<td class='d-temp'>"+jp.options[i][k]+"</td>");
			$('#table_toggle').show();
		}
	} else {
		topalert("Please check the format of the imported file.", "danger");
...
function parse_json(xraw,paused) {
    $('.bcomplex').remove(); //mf .bcomplex löscht doppelte mwb-Fenster
    var zraw = [];
    isparent = {};
	var tm;
    ...
    topalert("Decision tree JSON has been updated with "+raw.length+
	     ...
	     "use it now!","success")
	dt_clear()
	// dt_start() //mf anstelle von timeout... 250 in den anderen Funktionen
	...
	var options_html = x.options.reduce((h,r) => {
	    ...
	    var rlabel = r.label[0].toLocaleUpperCase()+r.label.substr(1).replace(/_/g, ' ');
	...
	if(x.label in isparent) {
	    ...
	    var mwbid = "mwb-"+hdiv;
	    $('body').append($('#mwb').clone().attr('id',mwbid).addClass('bcomplex')); //mf aus v5.1.5
	    var pmwbid = '#'+mwbid;
...
function update(source) {
	...
	.style("fill", function(d) {
		...
		var dname = d.name.replace(/\s/g, '_').replace(/\//g, '_or_').replace(/n\'t/g, '_not').split(":").shift().toLowerCase(); //mf .replace(...) ersetzt Leerzeichen beim Outcome/Entscheidung bei "Show Full Tree"!
		if(dname in lcolors) 
		    return undefined;
    ...
    */
	...
	.attr("nameid",function(d) { if(!d) return "1";
				     if(d.name) return d.name.replace(/\s/g, '_').replace(/\//g, '_or_').replace(/n\'t/g, '_not').split(":").pop().toLowerCase(); //mf (...).. ersetzt die nameid beim Kreis, ermöglicht Import von JSON-Tree mit Leerzeichen!
				   })
	.style("fill", function(d) {
	    if(d._children) return "lightsteelblue"
	    if(!('children' in d)) {
...
function update_links() {
    ...
    */
	...
	var xclass = "btext prechk-"+text.replace(/\s/g, '_').replace(/\//g, '_or_').replace(/n\'t/g, '_not').toLowerCase() //mf ermöglicht Klick durch clickprocess() auf "super_effective" etc. und "PoC"
	...
	d3.select("g#x"+id).append("text").attr("dx",-6).attr("dy",yoffset).attr("class","gtext")
	    ...
	    .text(text.replace(/_/g, ' ')).attr("startOffset",doffset+"%")	//mf Unterstrich bei Entscheidungen in GUI ersetzen
	    .on("click",pathclick)
	    .on("mouseover",showdiv)
	    .on("mouseout",hidediv);
...
function tmp_dismiss_modal(w) { /* Aus v5.1.5 übernommen */ //mf macht Show Cummulative funktionsfähig
    var current_modal_id = $('.modal.show').attr('id');
    $('.bcomplex').modal('hide');
    $('#tcummulative').data('mwbid',current_modal_id);
    $('#tcummulative').removeClass('d-none'); //mf nicht benötigtes Menü ausgeblendet lassen?
    setTimeout(function() {
	$('#tcummulative').addClass('d-none');
    },3000);
    
}
function show_complex() { /* Aus v5.1.5 übernommen */ //mf macht Show Cummulative funktionsfähig
